    ...
    if (participants && participants.length > 0) {
      suggestedTimes = (
        <View style={styles.center}>
          <Text style={styles.pTextBold}>
            Common Free Times for Participants:
          </Text>
          <Text>
            {suggestTimes(
              participants.concat(user),
              getUTCTime(this.state.datetime),
              getUTCTime(this.state.datetime1)
            ).map(timeInterval =>
              <View style={styles.pView}>
                <Text style={styles.pText}>
                  {convertLocalTime(timeInterval[0])} ~{" "}
                  {convertLocalTime(timeInterval[1])}
                </Text>
              </View>
            )}
          </Text>
      ...
      <ScrollView>
        ...
        >
          ...
          <View style={styles.container}>
            <Text>
              <View style={styles.padded}>
                <Text style={styles.graytext}>Private Event?</Text>
              </View>
              <Switch
          ...
          <DropDownPicker
            items={[
              {
                label: "exercise",
                value: "exercise",
                icon: () => <Icon name="plus-square" size={18} color="#900" />
              },
              {
                label: "rest",
                value: "rest",
            ...
            onChangeItem={item => {
              this.setState({ eventType: item.value });
